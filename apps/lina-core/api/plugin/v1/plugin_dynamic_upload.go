...
type UploadDynamicPackageReq struct {
	...
	OverwriteSupport statusflag.YesNo `json:"overwriteSupport" dc:"Whether to allow overwriting of dynamic plugin files with the same ID that have not been installed yet: 1=allowed 0=not allowed, default 0" eg:"0"`
}
...
type UploadDynamicPackageRes struct {
	...
	Installed   statusflag.Installation `json:"installed" dc:"Installation status: 1=installed 0=not installed; the default is still not installed after uploading" eg:"0"`
	Enabled     statusflag.Enabled      `json:"enabled" dc:"Enabled status: 1=enabled 0=disabled; the default is still disabled after uploading" eg:"0"`
}
