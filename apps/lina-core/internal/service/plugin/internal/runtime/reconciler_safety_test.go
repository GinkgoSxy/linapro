...
func TestRequiredRuntimeReconcilerLockReturnsConflict(t *testing.T) {
	...
	err = service.reconcilePrimaryPluginWithRequiredLock(
		ctx,
		&entity.SysPlugin{PluginId: pluginID, Type: catalog.TypeDynamic.String()},
		func(context.Context, *entity.SysPlugin) error {
			called = true
			return nil
		},
	)
...
func TestRecoverStaleReconcilingRestoresOldState(t *testing.T) {
	...
	registry, err := services.Catalog.SyncManifest(ctx, manifest)
	if err != nil {
	...
	if restored == nil || restored.CurrentState != catalog.HostStateEnabled.String() {
...
func TestRecoverFreshReconcilingSkipsPrimaryWork(t *testing.T) {
	...
	latest, err := services.Catalog.GetRegistry(ctx, pluginID)
	if err != nil {
		t.Fatalf("reload latest registry failed: %v", err)
	}
	if latest.CurrentState != catalog.HostStateReconciling.String() {
...
func TestRollbackInstallOrUpgradeReturnsRollbackDiagnostics(t *testing.T) {
	...
	failedRelease, err := services.Catalog.GetRelease(ctx, pluginID, manifest.Version)
	if err != nil {
	...
	reconcileErr := errors.New("original install failure")
	...
	releaseAfter, err := services.Catalog.GetReleaseByID(ctx, failedRelease.Id)
	if err != nil {
		t.Fatalf("expected release reload to succeed, got error: %v", err)
	}
	if releaseAfter.Status != catalog.ReleaseStatusFailed.String() {
	...
	if registryAfter.CurrentState != catalog.HostStateFailed.String() {
...
func newRuntimeSafetyServices() *runtimeSafetyServices {
	...
	runtimeSvc := New(catalogSvc, lifecycleSvc, nil, nil, nil, locker.New()).(*serviceImpl)
